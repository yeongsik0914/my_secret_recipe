...
### [ISSUE-018] 냉장고 파먹기 타이틀 타이포그래피 정돈, 상단 입력 섹션 비율 균형 및 원클릭 추천 칩 배치, 선반별 보관함 신선도 컬러 도트 설명 바 추가, 수량 단위 규격화 (g은 10g 단위, 그 외 1 단위 정수화)
...
### [ISSUE-019] 직접 식재료 입력 시 숫자만 입력해도 식재료 맞춤 표준 조리/보관 단위 자동 지정 및 기존 데이터 자가 교정
