...
## 5. 팀 협업 및 개발자별 기여 표기 표준 (Collaboration Standards)

본 저장소는 3인의 개발자가 협업 중이며, 향후 모든 개발 및 유지보수 작업 시 아래 표준을 필수로 준수합니다.

1. **`README.md` 업데이트 히스토리 (Changelog) 기록**:
   - 신규 기능 추가 또는 버전 릴리즈 시, `개발자 (Author)` 컬럼에 작업자의 GitHub 사용자명(`@GitHub_ID`)을 반드시 명시합니다.
2. **`log.md` 이슈 및 개선 내역 추적**:
   - 모든 기술적 문제 해결, 버그 핫픽스, 아키텍처 개편 시 `log.md`에 신규 이슈 항목(`[ISSUE-XXX]`)을 추가하고, `- **담당 개발자**: @GitHub_ID` 항목을 필수로 기록합니다.
3. **Git 커밋 및 브랜치 관리**:
   - Conventional Commits 형식(`feat:`, `fix:`, `refactor:`, `docs:` 등)을 준수하며 각 작업자의 기여도를 명확히 파악할 수 있도록 작성합니다.
