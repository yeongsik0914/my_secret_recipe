...
|---|---|---|---|
| **v1.9.4** | 2026-09-19 | [@yeongsik0914](https://github.com/yeongsik0914) | - **관리자 콘솔 회원 삭제 오류 알림 버그 완벽 수정 및 회원 목록 8명(8칸) 단위 5페이지 블록(1~5, 6~10) 페이지네이션 시스템 구축**:<br/>  1. **회원 삭제 오류 알림의 근본 원인(`ReferenceError: data is not defined`) 완벽 픽스**: 스토어 이벤트 구독 리스너에서 선언되지 않은 `data` 변수를 참조하던 치명적 결함을 `payload`로 수정하고, `store.notify()` 내 개별 구독자 `try-catch` 격리를 적용하여 삭제 성공 후 잘못된 Alert 팝업이 노출되던 문제를 완전 종식<br/>  2. **관리자 회원 목록 8명(8칸) 단위 슬라이스 렌더링**: 1페이지당 8명의 회원만 테이블에 정갈하게 노출하여 긴 스크롤 방지 및 가독성 대폭 향상<br/>  3. **5페이지 단위 블록(1~5, 6~10) 페이지네이션 컨트롤 바 구축**: 유저 수가 8명 초과 시 테이블 하단에 5개 단위 번호 버튼과 `[◀ 이전]`, `[다음 ▶]` 네비게이션 버튼을 동적 생성하여 직관적인 페이지 이동 지원 (5페이지에서 다음 클릭 시 6~10 블록으로 자동 전환, 6페이지에서 이전 클릭 시 1~5 블록으로 자동 전환)<br/>  4. **검색/필터 및 삭제 시 자동 페이지 보정 연동**: 이름/이메일 검색 및 권한/상태 필터 변경 시 1페이지로 자동 리셋, 회원 삭제로 인해 현재 페이지에 유저가 없을 경우 직전 페이지(`maxPage`)로 안전하게 자동 보정<br/>  5. **프리미엄 UI/UX 및 반응형 스타일링 완비**: 에메랄드 그린 브랜드 컬러와 활성 번호 하이라이트, 모바일 터치 최적화 레이아웃 완성 |
